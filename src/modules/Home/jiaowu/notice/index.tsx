...
import { cleanNoticeHtml, NoticeItem } from '@/jiaowu/jiaowuInfo/noticeInfo';
...
export default observer(function JiaowuNotice() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const cleaned = cleanNoticeHtml(resp.html);
      setList(cleaned.list || []);
    } catch (e) {
      const err = e as { message?: string } | null;
      setError(err?.message || '获取教务通知失败');
      setList([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 8, backgroundColor: 'transparent' }]}>
        ...
        <Text style={[styles.title, { color: theme.colors.onBackground }]}>教务通知</Text>
